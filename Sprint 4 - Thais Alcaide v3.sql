...
SELECT 
  t.id, t.declined,
  SUBSTRING_INDEX(SUBSTRING_INDEX(t.product_ids, ',', Num_productes.r), ',', -1) AS product_id_indiv
FROM 
  transactions t
JOIN 
  (SELECT 1 AS r UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4) AS Num_productes
ON 
  CHAR_LENGTH(t.product_ids) - CHAR_LENGTH(REPLACE(t.product_ids, ',', '')) >= Num_productes.r - 1;

UPDATE orders 
SET product_id_indiv = LTRIM(RTRIM(product_id_indiv));

     
-- Necessitem conèixer el nombre de vegades que s'ha venut cada producte.
SELECT o.product_id_indiv, p.product_name, count(o.product_id_indiv) AS total_vendes
FROM orders o
JOIN products p ON o.product_id_indiv = p.id
WHERE declined=0
GROUP BY p.id
